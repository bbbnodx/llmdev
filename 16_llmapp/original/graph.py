...
memory = MemorySaver()  # MemorySaverインスタンスの作成
graph = None
...
def create_index(persist_directory, embedding_model):
    # 現在実行中のスクリプトパスからdataへの相対パスを取得し、テキストファイルを読み込む
    current_script_path = os.path.abspath(__file__)
    curent_directory = os.path.dirname(current_script_path)

    loader = DirectoryLoader(f"{curent_directory}/data/pdf", glob="./*.pdf",   loader_cls=PyPDFLoader)
...
def stream_graph_updates(graph: StateGraph, user_message: str, thread_id) -> str:
    ...
    response = graph.invoke(
        {"messages": [("system", "あなたは猫です。猫らしく語尾などに「にゃ」と付けて喋ります"),
                      ("user", user_message)]},
